...
$(document).ready(function() {
	...
	var mainOpts = {
		...
		content: '<iframe id="mainFrame" name="mainFrame" src="/dataV" allowTransparency="true" style="border: 0; width: 100%; height: 99%;" frameBorder="0"></iframe>'
	};
...
function btnFault(){
	if (mainTabs.tabs('exists','故障位置')){
		mainTabs.tabs('select','故障位置');
	} else {
		var content = '<iframe scrolling="auto" frameborder="0"  src="/dataV/show" style="width:100%;height:100%;"></iframe>';
		mainTabs.tabs('add',{
			fit: true,
			closable: true,
			border: false,
			iconCls: 'eova-icon263',
			title:'小车位置',
			content:content,
		});
	}
